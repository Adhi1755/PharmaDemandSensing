                ...
                <div className="scroll-section border border-[rgba(255,255,255,0.08)] bg-[#1D1E27]">
                    ...
                    <section className="border-b border-[rgba(255,255,255,0.08)]">
                        <div className="grid grid-cols-1 gap-0 sm:grid-cols-2 xl:grid-cols-4">
                            <div className="border-b border-[rgba(255,255,255,0.08)] p-6 sm:border-r xl:border-b-0">
                                <p className="text-xs uppercase tracking-widest text-[rgba(224,226,228,0.72)] mb-2">Total Drugs</p>
                                <p className="text-3xl font-semibold text-[var(--color-light-gray)]">{stats?.totalDrugs ?? 0}</p>
                                <p className="text-xs text-[rgba(224,226,228,0.62)] mt-1">Active pharmaceutical products</p>
                            </div>
                            <div className="border-b border-[rgba(255,255,255,0.08)] p-6 xl:border-r xl:border-b-0">
                                <p className="text-xs uppercase tracking-widest text-[rgba(224,226,228,0.72)] mb-2">Predicted Demand (7d)</p>
                                <p className="text-3xl font-semibold text-[var(--color-light-gray)]">{stats?.predictedDemand7d?.toLocaleString() ?? "0"}</p>
                                <p className="text-xs text-[rgba(224,226,228,0.62)] mt-1">Units forecasted for next week</p>
                            </div>
                            <div className="border-b border-[rgba(255,255,255,0.08)] p-6 sm:border-r xl:border-b-0 xl:border-r">
                                <p className="text-xs uppercase tracking-widest text-[rgba(224,226,228,0.72)] mb-2">High Demand Alerts</p>
                                <p className="text-3xl font-semibold text-[var(--color-light-gray)]">{stats?.highDemandAlerts ?? 0}</p>
                                <p className="text-xs text-[rgba(224,226,228,0.62)] mt-1">Drugs needing attention</p>
                            </div>
                            <div className="p-6">
                                <p className="text-xs uppercase tracking-widest text-[rgba(224,226,228,0.72)] mb-2">Forecast Accuracy</p>
                                <p className="text-3xl font-semibold text-[var(--color-light-gray)]">{`${stats?.avgForecastAccuracy ?? 0}%`}</p>
                                <p className="text-xs text-[rgba(224,226,228,0.62)] mt-1">TFT model performance</p>
                            </div>
                        </div>
                    </section>

                    {/* Row 2: Chart + Side Panel */}
                    <section className="grid grid-cols-1 gap-0 border-b border-[rgba(255,255,255,0.08)] xl:grid-cols-12">
                        <div className="xl:col-span-8 p-6 lg:p-8 border-b border-[rgba(255,255,255,0.08)] xl:border-b-0 xl:border-r">
                        ...
                        <div className="xl:col-span-4 p-6 lg:p-8">
                            ...
                            <div className="border border-[rgba(255,255,255,0.08)]">
                                {topDrugs.map((drug, i) => (
                                    <div
                                        key={drug.id}
                                        className="flex items-center gap-4 p-3 border-b border-[rgba(255,255,255,0.08)] last:border-b-0 bg-[rgba(13,15,18,0.45)] hover:bg-[rgba(13,15,18,0.65)] transition-colors"
                                    >
                                        <span className="w-8 h-8 flex items-center justify-center text-[var(--color-light-gray)] font-bold text-sm shrink-0 border border-[rgba(255,255,255,0.08)] bg-[rgba(255,0,0,0.18)]">
                                            {i + 1}
                                        </span>
                    ...
                    <section className="p-6 lg:p-8">
                            ...
                            <div className="grid grid-cols-1 gap-0 md:grid-cols-2 border border-[rgba(255,255,255,0.08)]">
                                {alerts.map((alert, i) => (
                                    <div
                                        key={i}
                                        className={`border-b border-r border-[rgba(255,255,255,0.08)] p-4 md:[&:nth-child(2n)]:border-r-0 [&:nth-last-child(-n+2)]:md:border-b-0 [&:last-child]:border-b-0 ${alertTypeStyles[alert.type] || "border-l-[rgba(224,226,228,0.6)] bg-[rgba(224,226,228,0.08)]"}`}
                                    >
